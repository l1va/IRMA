...
let staticServe = function (req, res) {
  ...
  var ignoreFromFile = false;

  if (req.url === '/') {
    req.url = '/index.html';
  } else if (req.url == '/get-api-urls') {
    ignoreFromFile = true;
    var editorDomain = process.env.DOMAIN || 'localhost';
    var editorPort = process.env.PORT_EDITOR || '8000';
    var editorUrl = `http://${editorDomain}:${editorPort}/api/get-urls`;
    console.log('Connecting to: ' + editorUrl);
    http.get(editorUrl, (resp) => {
    let data = '';
